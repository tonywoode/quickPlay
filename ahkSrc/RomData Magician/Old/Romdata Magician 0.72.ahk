...
InitializeVars()
{
 ...
 IniRead, SelectedIni, %ScriptIni%, Paths, LastIni
 If SelectedIni = Error
    SelectedIni =
...
CreateGUIs()
{
 ...
 ListViewHeight := A_ScreenHeight-410
...
; Name|Ini Name|ParentName|ZipName|path|Emulator|Company|Year|Game Type|Rating|Language|Parameters|Comment|NumPlayed|ParamMode|<IPS>|IPS1|IPS2|IPS3|</IPS>|Players|DefaultGoodMerge|System
 ...
 Gui, Add, Text, x30 y+35, System Ini file:
 Gui, Add, Edit, yp-3 x100 W450 r1 veIni, %SelectedIni%
 Gui, Add, Button, yp x+10 gGetIniFile, ...
...
GetIniFile:
 Gui +OwnDialogs
 GuiControlGet, LastIni,, eIni
 SplitPath, LastIni,, Dir
 FileSelectFile, SelectedIni,, %Dir%, Select System Ini file, *.ini
 If SelectedIni =
    MsgBox, No File Selected
 Else
 {
  GuiControl,, eIni, %SelectedIni%
  IniWrite, %SelectedIni%, %ScriptIni%, Paths, LastIni
 }
Return
...
; GuiControl, -Redraw, QuickPlayLV
 Loop % LV_GetCount()
 {
  ...
  PlayersCol++
  SystemCol := PlayersCol+2
  Section := Varize(Title)
  LV_GetText(System, RowNumber, SystemCol)
  If System Not in %SystemList%
  {
   ...
   If SystemIniFilePath = Error
      SystemIniFilePath = %QPFolder%\dats\System Ini Files
   FileRead, FileContent, %SystemIniFilePath%\%IniFile%.ini

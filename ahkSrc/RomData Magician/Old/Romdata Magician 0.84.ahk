...
InitializeVars()
{
 ...
 FileRead, Contents, %QPFolder%\dats\emulators.ini
 SystemNames := ini_getAllSectionNames(Contents)
 Loop, Parse, SystemNames, `,
 {
  LoopField := A_LoopField
  Ini_Read(System, Contents, A_LoopField, "system")
  ...
  If (System <> "ERROR") AND (System <> "")
  {
   ...
   Loop, Parse, IniPaths, `n
   {
    If GoodName <> Error
    {
     IfInString, A_LoopField, %GoodName% ;Change to SplitString?
     {
...
; Name|Ini Name|ParentName|ZipName|path|Emulator|Company|Year|Game Type|Rating|Language|Parameters|Comment|NumPlayed|ParamMode|<IPS>|IPS1|IPS2|IPS3|</IPS>|Players|DefaultGoodMerge|System
 ...
 ColWidth =   300,200,0,0,0,200,150,70,90,70,70,0,150,50,0,0,0,0,0,0,100,50,50
 Loop, Parse, ColWidth, `,
 ...
 Gui, Add, CheckBox, x30 yp+40 vAddRating Checked%AddRating%, Add Rating
...
; GuiControl, -Redraw, QuickPlayLV
 Loop % LV_GetCount()
 {
  ...
  LV_GetText(System, RowNumber, 23)
  If Title =
     Continue
...
LoadRomData:
 ...
 Loop, Parse, RomDataDat, `n, `r
 {
  ...
  ;%Emulator% := System  (Ini File Name)
   Emulator := Varize(Data6)
   System := %Emulator%
   vSystem := Varize(System)
  If Emulator Not In %EmulatorList%
  {
   EmulatorList := EmulatorList = "" ? Emulator : "," Emulator
  ...
  SB_SetText("   Parsing RomData.dat " A_LoopField)
...
;   MsgBox, 0, %ScriptName%, %Data1%`n%TempVar%
  ...
  LV_Add("",Data1,Data2,Data3,Data4,Data5,Data6,Data7,Data8,Data9,Data10,Data11,Data12,Data13
          ,Data14,Data15, "<IPS>", "", "", "", "</IPS>", Data21, Data22, System)
